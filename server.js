...
app.post('/register', (req, res) => {
    const {nombresApellidos, usuario, email, password, confirmPassword, tipoDocumento, documento} = req.body;
    const saltRounds = 10;
    const inserUserQuery = 'INSERT INTO usuarios (id_tipo_documento, documento, nombre, correo, usuario, clave) VALUES (?, ?, ?, ?, ?, ?)';

    // Verificar si el nombre de usuario ya esta en uso
    const checkUsernameQuery = 'SELECT * FROM usuarios WHERE correo = ? or usuario = ?';
    
    db.query(checkUsernameQuery, [email, usuario], (err, result) => {
        if(err){
            throw err;
        }
        ...
        bcrypt.hash(password, saltRounds, (err, hashedPassword) => {
            ...
            db.query(inserUserQuery, [tipoDocumento, documento, nombresApellidos, email, usuario, hashedPassword], (err, result) => {
                if(err) {
                    throw err;
                }
